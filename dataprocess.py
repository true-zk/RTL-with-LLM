...
class TACM12K(BaseLoadCls):
    ...
    @classmethod
    def load_all(cls):
        hgraph = HeteroGraphData.load(osp.join(cls.rag_dir, cls.rag_data_list[0]))
        author_text_file = osp.join(cls.rag_dir, cls.rag_data_list[1])
        paper_text_file = osp.join(cls.rag_dir, cls.rag_data_list[2])
        y = torch.load(osp.join(cls.rag_dir, cls.rag_data_list[-2]), weights_only=False)
        masks = torch.load(osp.join(cls.rag_dir, cls.rag_data_list[-1]), weights_only=False)
        return hgraph, author_text_file, paper_text_file, y, masks

    # sampled tape utility functions ##########################
    def pyg_graph(self):
        # Convert HeteroGraphData to PyG Data
        hgraph, _, _, _, _ = self.load_all()
        pyg_graph = HeteroData()
        for edge_type in hgraph.edge_types:
            edge_d = hgraph[edge_type].to_dict()
    ...
    def pyg_loader(self):
        ...
        loader = NeighborLoader(
            ...
            batch_size=1,
            shuffle=False,
            input_nodes=pyg_graph['paper'].node_id,
        )
        return loader
...
class TLF2K(BaseLoadCls):
    ...
    @classmethod
    def load_all(self):
        hgraph = HeteroGraphData.load(osp.join(self.rag_dir, self.rag_data_list[0]))
        artist_text_file = osp.join(self.rag_dir, self.rag_data_list[1])
        y = torch.load(osp.join(self.rag_dir, self.rag_data_list[-2]), weights_only=False)
        masks = torch.load(osp.join(self.rag_dir, self.rag_data_list[-1]), weights_only=False)
        return hgraph, artist_text_file, y, masks
...
class TML1M(BaseLoadCls):
    ...
    def load_all(self):
        hgraph = HeteroGraphData.load(osp.join(self.rag_dir, self.rag_data_list[0]))
        user_text_file = osp.join(self.rag_dir, self.rag_data_list[1])
        movie_text_file = osp.join(self.rag_dir, self.rag_data_list[2])
        y = torch.load(osp.join(self.rag_dir, self.rag_data_list[-2]), weights_only=False)
        masks = torch.load(osp.join(self.rag_dir, self.rag_data_list[-1]), weights_only=False)
        return hgraph, user_text_file, movie_text_file, y, masks
